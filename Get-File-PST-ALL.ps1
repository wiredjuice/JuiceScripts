$strComputers = Get-Content -Path "C:\ittemp\computernames.txt"
...
if($firstOutput)
{
Write-output $output | Out-File -Encoding ascii -filepath "C:\ittemp\ALLpstdetails.csv"
$firstOutput = $false
}
else
{
Write-output $output | Out-File -Encoding ascii -filepath "C:\ittemp\ALLpstdetails.csv" -append
}
}
}
}