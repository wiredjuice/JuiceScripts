$strComputers = Get-Content -Path "C:\ittemp\servernames.txt"
...
if($firstOutput)
{
Write-output $output | Out-File -Encoding ascii -filepath "C:\ittemp\ServerPSTdetails.csv"
$firstOutput = $false
}
else
{
Write-output $output | Out-File -Encoding ascii -filepath "C:\ittemp\ServerPSTdetails.csv" -append
}
}
}
}